...
<main role="main">
    <div class="container">
        <form class="form-inline" action="../ConsultCTReceiptServlet" method="post">
            <div class="form-group">

                <label for="CTID">收费员姓名</label>
                <select class="form-control" id="CTID" name="name">
                    <%DBConc db = new DBConc();
                        Map<String,String> ans = db.getCT();
    ...
    <div class="container">
        ...
        <div class="container">
            <%
                String ctname = (String)session.getAttribute("ctname");
                String starttime = (String)session.getAttribute("ctstart");
                String endtime = (String)session.getAttribute("ctend");
                if(ctname!=null && !((String) session.getAttribute("ctname")).isEmpty()){
            %>
            <h4><%=ctname%> 收费员<%=starttime%>至<%=endtime%>的收费情况</h4>
            <%
                    session.setAttribute("ctname","");
                    session.setAttribute("ctstart","");
                    session.setAttribute("ctend","");
            }
        ...
        <div class="table-responsive">
            ...
            >
                ...
                <tr>
                    ...
                    <th data-field="starttime" data-sortable="true">开票时间</th>
                    <th data-field="endtime" data-sortable="true">开票医生</th>
                    <th data-field="item" data-sortable="true">收费项目名称</th>
                ...
                <%
                    ArrayList<CTReceipt> ctReceipts = (ArrayList<CTReceipt>)session.getAttribute("ctreceipts");
                    double total = 0.0;
                    if(ctReceipts!=null){
                        DBConc dc = new DBConc();
                        for(int i=0;i<ctReceipts.size();i++){
                            String id = String.valueOf(ctReceipts.get(i).getReceipt_id());
                            String doctor = dc.getDoctorName(ctReceipts.get(i).getDoctor_id());
                            String date = String.valueOf(ctReceipts.get(i).getPays_date());
                            String item = dc.getItemName(ctReceipts.get(i).getItem_id());
                            String num = String.valueOf(ctReceipts.get(i).getNum());
                            total+=ctReceipts.get(i).getAmount();
                            String amount = String.valueOf(ctReceipts.get(i).getAmount());
                %><tr>
                <td><%=i+1%></td>
                <td><%=id%></td>
                <td><%=date%></td>
